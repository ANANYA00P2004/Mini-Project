  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setUserData(data.user || {}); // Ensure userData is an object
        setTotalExpenses(data.totalExpenses || 0);
        setBudgetUtilization(data.budgetUtilization || 0);
        setBudgetOverview(data.budgetOverview || { datasets: [] });
        setCategorySpending(data.categorySpending || []);
        setTopExpenseCategories(data.topExpenseCategories || []);
        setWeeklyComparison(data.weeklyComparison || { datasets: [] });
        setMonthlyComparison(data.monthlyComparison || { datasets: [] });
        setDailyTrend(data.dailyTrend || { datasets: [] });
        setFutureTimeline(data.futureTimeline || []);
      } catch (error) {
        ...
        <ul>
          {futureTimeline.length > 0 ? (
            futureTimeline.map((event, index) => (
              <li key={index}>
                {event.date} - {event.title} (₹ {event.expected_amount})
              </li>
            ))
          ) : (
